...
import { centralHeading } from './communication-header.mjs';
...
export function buildPortfolioReport({ dataset, entity, metric = "VN", includeBoth = false, month = 0, period = "ytd", uplift = 0 }) {
  ...
  const centralName = centralHeading(entity.central, dataset.registry?.entities.find(item => item.id === `central:${entity.central}`)?.name || (entity.kind === 'central' ? entity.name : selectedRows.find(row => row.centralName)?.centralName));
  return { version: 1, year: dataset.year, month, period, periodLabel: periodTitle(period, month, dataset.year), source, uplift, entity: { ...entity }, centralName, sections };
}

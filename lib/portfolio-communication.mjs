...
export function normalizeRecipients(values) {
  ...
  if (!Array.isArray(entries) || entries.length > 100) throw new Error("Informe até 100 e-mails nesta comunicação.");
...
export function buildEmailFile({ recipients = [], subject, text, html }) {
  ...
  return ["X-Unsent: 1", `To: ${normalizeRecipients(recipients).join(", ")}`, `Subject: =?UTF-8?B?${base64(safeSubject(subject))}?=`, "MIME-Version: 1.0", `Content-Type: multipart/alternative; boundary="${boundary}"`, "", `--${boundary}`, "Content-Type: text/plain; charset=UTF-8", "Content-Transfer-Encoding: base64", "", mimeBody(text), `--${boundary}`, "Content-Type: text/html; charset=UTF-8", "Content-Transfer-Encoding: base64", "", mimeBody(html), `--${boundary}--`, ""].join("\r\n");
}

...
export function dashboardImageLayout(model, context) {
  ...
  y += text('GESTÃO COMERCIAL', inset, y, contentWidth, 20, true, '#8fdbcf') + 12;
  y += text(model.scope, inset, y, contentWidth, 34, true, '#ffffff') + 10;
  y += text(model.periodLabel, inset, y, contentWidth, 28, true, '#ffffff') + 8;
  if (model.hierarchy) y += text(model.hierarchy, inset, y, contentWidth, 21, false, '#ffffff') + 8;
  for (const note of model.notes) y += text(note, inset, y, contentWidth, 21, false, '#c4dedf') + 6;
  y += 16; commands[headerIndex].height = y;
  y += 12;

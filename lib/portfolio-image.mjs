import { dashboardHeader, validateDashboard } from './portfolio-presentation.mjs';
...
export function dashboardImageLayout(model, context) {
  ...
  y += text(header.brand, inset, y, contentWidth, 20, true, '#8fdbcf') + 6;
  y += text(header.centralName, inset, y, contentWidth, 30, true, '#ffffff') + 6;
  y += text(header.periodLabel, inset, y, contentWidth, 23, true, '#ffffff') + 12;
  commands[headerIndex].height = y;
  y += 12;
  const unit = [header.unitLabel, header.unitContext].filter(Boolean).join(' · ');
  if (unit) y += text(unit, inset, y, contentWidth, 24, true) + 6;
  if (model.notes.length) y += text(model.notes.join(' · '), inset, y, contentWidth, 20, false, '#435c60') + 8;
  if (model.greeting) y += text(model.greeting, inset, y, contentWidth, 27, true) + 8;
  if (model.opening) y += text(model.opening, inset, y, contentWidth, 26) + 10;
  for (const b of model.blocks) {
    if (b.type === 'heading') { y += 14; y += text(b.text, inset, y, contentWidth, 29, true) + 8; }
    else if (b.type === 'text') {
      const special = ['action', 'warning'].includes(b.tone), padding = special ? 16 : 0, top = y;
      const index = commands.length;
      if (special) rect(inset, y, contentWidth, 1, b.tone === 'action' ? '#e6f6f2' : '#fff8e7');
      y += padding + text(b.text, inset + padding, y + padding, contentWidth - padding * 2, b.tone === 'muted' ? 21 : 24, b.tone === 'action', b.tone === 'warning' ? '#704700' : b.tone === 'muted' ? '#435c60' : '#003641') + padding;
      ...
      for (let i = 0; i < b.items.length;) {
        ...
        if (i >= 3 && row.length === 1 && projectionSize != null) {
          ...
          const labelHeight = text(item.label, inset + 18, top + 14, widths[0], 23, false, item.accent ? '#c4dedf' : '#435c60');
          const valueHeight = valueText(item.value, inset + 18 + widths[0] + 16, top + 14, widths[1], projectionSize, item.accent ? '#ffffff' : '#003641');
          const supportHeight = item.support ? text(item.support, inset + 18 + widths[0] + widths[1] + 32, top + 14, widths[2], 24, !!item.accent, item.accent ? '#ffffff' : '#435c60') : 0;
        ...
        const h = 32 + labelHeight + valueHeight + (supportHeight ? supportHeight + 8 : 0);
        row.forEach((item, col) => {
          const x = inset + col * (w + gap);
          rect(x, y, w, h, item.accent ? '#003641' : '#f0f7f4');
          text(item.label, x + 18, y + 14, w - 36, 23, false, item.accent ? '#c4dedf' : '#435c60');
          valueText(item.value, x + 18, y + 20 + labelHeight, w - 36, valueSize, item.accent ? '#ffffff' : '#003641');
          if (item.support) text(item.support, x + 18, y + 28 + labelHeight + valueHeight, w - 36, 26, !!item.accent, item.accent ? '#ffffff' : '#435c60');
        });
        y += h + gap;
        i += columns;
      }
  ...
  if (model.footer) y += text(model.footer, inset, y + 8, contentWidth, 21, false, '#435c60') + 14;

  ...
  for (const b of model.blocks) {
    if (b.type === 'heading') { y += 24; y += text(b.text, inset, y, contentWidth, 34, true) + 14; }
      ...
      for (let i = 0; i < b.items.length; i += 2) {
        ...
        const w = pair.length === 1 ? contentWidth : (contentWidth - gap) / 2;
        const h = Math.max(...pair.map((item) => lines(item.label, w - 36, 23, false).length * 33 + lines(item.value, w - 36, 36, true).length * 52 + (item.support ? lines(item.support, w - 36, 26, !!item.accent).length * 37 + 12 : 0) + 50));
        pair.forEach((item, col) => {
          const x = inset + col * (w + gap);
          rect(x, y, w, h, item.accent ? '#003641' : '#f0f7f4');
          const labelHeight = text(item.label, x + 18, y + 20, w - 36, 23, false, item.accent ? '#c4dedf' : '#435c60');
          const valueHeight = text(item.value, x + 18, y + 30 + labelHeight, w - 36, 36, true, item.accent ? '#ffffff' : '#003641');

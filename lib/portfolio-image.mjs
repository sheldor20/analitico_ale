...
export function dashboardImageLayout(model, context) {
  ...
  function fittedValueSize(items, maxWidth) {
    // Keep a common size within the row. Longer values still wrap at the floor.
    for (let size = 36; size >= 26; size--) {
      context.font = `700 ${size}px "Sicoob Sans", Arial`;
      if (items.every((item) => context.measureText(item.value).width <= maxWidth)) return size;
    }
    return 26;
  }
    ...
    if (b.type === 'heading') { y += 24; y += text(b.text, inset, y, contentWidth, 34, true) + 14; }
      ...
      for (let i = 0; i < b.items.length; i += 3) {
        ...
        if (i > 0 && row.length === 1) {
          ...
          const valueHeight = text(item.value, inset + 18 + widths[0] + 16, top + 18, widths[1], fittedValueSize([item], widths[1]), true, item.accent ? '#ffffff' : '#003641');
          ...
          commands[index].height = h; y += h + gap; continue;
        }
        ...
        const valueHeight = Math.max(...row.map((item) => lines(item.value, w - 36, valueSize, true).length * Math.ceil(valueSize * 1.42)));
        ...
        row.forEach((item, col) => {
          ...
          text(item.value, x + 18, y + 26 + labelHeight, w - 36, valueSize, true, item.accent ? '#ffffff' : '#003641');

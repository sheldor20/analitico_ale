...
export const SORT_OPTIONS = Object.freeze({ gap: 'Maior GAP', attainment: 'Menor atingimento', 'attainment-desc': 'Maior atingimento', production: 'Maior produção', contribution: 'Maior contribuição', evolution: 'Maior evolução · 3 meses fechados', 'projected-gap': 'Maior GAP projetado', name: 'Nome / código' });
...
export function sortAnalysis(rows, order = 'gap') {
  return [...rows].sort((a, b) => {
    ...
    const field = order === 'production' ? 'actual' : order === 'contribution' ? 'contribution' : order === 'evolution' ? 'recentGrowth' : order === 'projected-gap' ? 'projectionGap' : order.startsWith('attainment') ? 'attainment' : 'gap';
    const av = a[field];
    const bv = b[field];
...
const matchesScope = (row, filters) => (filters.central === 'all' || !filters.central || row.central === filters.central) && (filters.coop === 'all' || !filters.coop || `${row.central}:${row.cooperative}` === filters.coop) && (!filters.pa || filters.pa === 'all' || (row.kind === 'central' ? filters.pa.startsWith(`${row.central}:`) : row.kind === 'cooperative' ? filters.pa.startsWith(`${row.central}:${row.cooperative}:`) : `${row.central}:${row.cooperative}:${row.pa}` === filters.pa));
const matchesText = (row, search = '') => `${row.name || ''} ${row.central || ''} ${row.cooperative || ''} ${row.pa ?? ''}`.toLocaleLowerCase('pt-BR').includes(search.trim().toLocaleLowerCase('pt-BR'));
...
export function scopedAnalyses(dataset, filters = {}) {
  ...
  return analysisIndicators(aggregate(rows, source === 'cadence' ? 'pa' : filters.level || 'cooperative').map(row => analyze(row, optionsFor(dataset, filters))), optionsFor(dataset, filters));
}
...
export function networkSummary(dataset, filters = {}) {
  ...
  const selected = scopedAnalyses(dataset, filters).filter(row => matchesText(row, filters.search) && matchesStatus(row, filters.status) && (!filters.unitIds || filters.unitIds.includes(identity(row, filters.source === 'cadence' ? 'pa' : filters.level || 'cooperative'))));
  ...
  if (filters.unitIds || filters.search || (filters.status && filters.status !== 'all')) entities = entities.filter(entity => keys.has(`central:${entity.central}`) || keys.has(`cooperative:${entity.central}:${entity.cooperative}`) || keys.has(entity.id));
...
/** Compare the same closed months for every unit; never use projected production. */
export function analysisIndicators(rows, { year, month = 0, period = 'month' }) {
  const range = periodBounds(year, month, period);
  const observedEnd = row => [row.cutoffMin || row.cutoff, row.cutoff].map(date => date < range.end ? date : range.end);
  const dates = new Set(rows.flatMap(observedEnd));
  const moneySum = values => values.reduce((total, value) => total + Math.round(value * 100), 0) / 100;
  const total = moneySum(rows.map(row => valid(row.actual) ? row.actual : 0));
  const comparable = rows.length > 0 && dates.size === 1 && total > 0 && rows.every(row => row.complete && valid(row.actual) && !row.annualConflict);
  const last = Math.min(range.last, ...rows.flatMap(row => [lastClosedMonth(row), lastClosedMonth({ cutoff: row.cutoffMin || row.cutoff })]));
  const recentLabel = last >= 5 ? `${String(last - 1).padStart(2, '0')}–${String(last + 1).padStart(2, '0')}/${year} vs. ${String(last - 4).padStart(2, '0')}–${String(last - 2).padStart(2, '0')}/${year}` : 'Requer seis meses fechados';
  return rows.map(row => {
    const values = last >= 5 ? row.actuals?.slice(last - 5, last + 1) : [];
    const complete = values?.length === 6 && values.every(valid) && period !== 'daily';
    const previous = complete ? moneySum(values.slice(0, 3)) : null;
    const current = complete ? moneySum(values.slice(3)) : null;
    return { ...row, contribution: comparable ? Math.round(row.actual * 100) / 100 / total : null, recentGrowth: previous > 0 ? (Math.round(current * 100) - Math.round(previous * 100)) / (previous * 100) : null, recentLabel };
  });
}

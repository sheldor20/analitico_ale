...
export function mergeProduction(current, incoming, { goalsOnly = false } = {}) {
  if (!current) {
    ...
    if (goalsOnly) initial.rows.forEach((row) => { row.actuals = Array(12).fill(null); });
  ...
  for (const next of update.rows) {
    ...
    if (!previous) {
      ...
      next.actuals = next.actuals.map((value, month) => !goalsOnly && month <= last ? value : null);
      records.set(key, next);
      continue;
    }
    if (goalsOnly) continue;

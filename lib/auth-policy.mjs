...
export function sameOrigin(request) {
  try {
    ...
    if (!raw || (site && site !== 'same-origin')) return false;
    const origin = new URL(raw);
    if (raw !== origin.origin) return false;
    // Next can normalize request.url to an internal localhost origin behind Proxy.
    // Host is the destination authority chosen by the browser; never authorize from
    // a caller-supplied X-Forwarded-Host or Referer instead of Origin.
    const host = request.headers.get('host') || new URL(request.url).host;
    if (origin.host !== host.toLowerCase()) return false;

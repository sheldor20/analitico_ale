...
export function analyze(row, { year, month, period = "month", uplift = 0 }) {
  ...
  const targetValues = row.targets.slice(range.first, range.last + 1);
  const monthlyTarget = targetValues.every((v) => v !== null)
    ? sum(targetValues)
    : null;
  const target =
    period === "annual" ? (row.annualTarget ?? null) : monthlyTarget;
  ...
  for (let m = range.first; m <= range.last; m++) {
    ...
    if (row.targets[m] == null) missing = true;
    else expected += row.targets[m] * fraction;
  }
  ...
  return {
    ...
    dailyActual: null,
    dailyTarget:
      row.targets[month] == null
        ? null
        : row.targets[month] / businessDays(...bounds(year, month)),
  };
}
export function aggregate(rows, level) {
  ...
  for (const row of rows) {
    ...
    g.members++;
    g.annualTarget =
      g.annualTarget == null || row.annualTarget == null
        ? null
        : g.annualTarget + row.annualTarget;
    // Null propagates so partial coverage is never silently reported as complete.
    for (let m = 0; m < 12; m++) {
      g.targets[m] =
        g.targets[m] == null || row.targets[m] == null
          ? null
          : g.targets[m] + row.targets[m];
      g.actuals[m] =
        g.actuals[m] == null || row.actuals[m] == null
          ? null
...
export function reconcile(rows) {
  const notes = [];
  const pas = rows.filter((r) => r.source === "cadence");
  for (const coop of rows.filter(
    (r) => r.source === "base" && r.metric === "VN",
  )) {
    const children = pas.filter(
      (p) => p.central === coop.central && p.cooperative === coop.cooperative,
    );
    if (!children.length) continue;
    const limit = Math.min(
      day(coop.cutoff).getUTCMonth(),
      day(children[0].cutoff).getUTCMonth(),
    );
    for (let m = 0; m <= limit; m++) {
      ...
      if (Math.abs(value) > 0.011)
        notes.push({
          kind: "reconciliation",
          message: `${coop.cooperative} · ${MONTHS[m]}: PAs menos cooperativa = ${money(value)}${coop.cutoff !== children[0].cutoff ? " (fontes com cortes diferentes)" : ""}.`,

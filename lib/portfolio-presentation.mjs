...
function annualLine(section) {
  ...
  return `${section.label}: meta ${money(a.target)}; realizado ${money(a.actual)} (${percent(a.attainment)}); projeção ${money(a.projected)}; GAP ${money(a.gap)}.${warning(a) ? ` ${warning(a)}` : ''}`;
}
...
export function buildPortfolioPresentation(report, { names = [], intro = '', signature = '', subject = '' } = {}) {
  ...
  for (const section of report.sections) {
    ...
    const variance = goalVariance(v.actual, v.target, v.complete && !v.annualConflict);
    ...
    blocks.push({ type: 'cards', items: [
      { label: report.period === 'annual' ? 'Meta anual' : 'Meta do período', value: money(v.target), support: selectedTitle },
      { label: 'Realizado informado', value: money(v.actual), support: `${percent(v.attainment)} da meta`, accent: true },
      { label: variance.label, value: money(variance.value), support: varianceSupport },
      { label: v.phase === 'Fechado' ? 'Fechamento apurado' : 'Projeção de fechamento', value: money(v.projected), support: v.phase === 'Fechado' ? 'Resultado fechado' : `${percent(v.projectedAttainment)} da meta · estimativa` },
    ] });
    for (const text of details.slice(1)) blocks.push({ type: 'text', text, tone: text === warning(v) ? 'warning' : 'body' });

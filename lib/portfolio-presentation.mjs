...
/** Optional presentation metadata keeps saved v2 dashboards compatible. */
export function dashboardHeader(model) {
  const identity = model.entityId.match(/^(central|cooperative|pa):(\d+)/);
  const central = identity?.[2] || model.hierarchy.match(/Central (\d+)/)?.[1];
  const isCentral = identity?.[1] === 'central';
  const centralName = model.header?.centralName || centralHeading(central, isCentral ? model.scope.replace(/^Central \d+\s*·\s*/, '') : '');
  const metricLabel = model.header?.metricLabel ?? metricHeading(model.blocks.filter(block => block.type === 'heading').map(block => block.text));
  return { brand: communicationBrand(metricLabel), centralName, periodLabel: model.periodLabel,
    unitLabel: isCentral ? '' : model.scope, unitContext: model.hierarchy.replace(/^Central \d+(?:\s*·\s*)?/, '').trim() };
}
...
export function buildPortfolioPresentation(report, { names = [], intro = '', signature = '', subject = '', showProjection = false, showAnnual = true } = {}) {
  ...
  for (const section of report.sections) {
    if (report.sections.length > 1) blocks.push({ type: 'heading', text: section.label });
  ...
  if (showAnnual && report.period !== 'annual') {
    ...
    for (const section of report.sections) {
      if (report.sections.length > 1) blocks.push({ type: 'heading', text: `${section.label} · ano` });
      if (!section.available) {
  ...
  const dashboard = { version: 2, year: report.year, period: report.period, entityId: entity.id,
    ...
    header: { centralName: centralHeading(entity.central, report.centralName || (entity.kind === 'central' ? entity.name : '')), metricLabel: metricHeading(report.sections.map(section => section.metric)) } };
  const visualHeader = dashboardHeader(dashboard);
  const header = [visualHeader.brand, visualHeader.centralName, selectedTitle, [visualHeader.unitLabel, visualHeader.unitContext].filter(Boolean).join(' · '), ...notes].filter(Boolean);
  ...
  const whatsapp = [...(greeting ? [greeting, ''] : []), ...header, ...(opening ? ['', opening] : []), ...primaryWhatsapp, ...annual, ...end].join('\n');
...
export function validateDashboard(model) {
  ...
  if (model.header !== undefined && (!model.header || typeof model.header !== 'object' || !string(model.header.centralName) || !string(model.header.metricLabel, 100))) throw new Error('Cabeçalho do painel inválido. Gere novamente o cenário.');
...
export function renderDashboardHtml(model, subject = '') {
  ...
  const header = dashboardHeader(model);
  ...
  return `<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escape(subject || model.scope)}</title><style>@media(max-width:480px){.panel-content{padding:14px!important}.metric-cell{padding:12px 8px!important}.metric-label{font-size:12px!important;min-height:36px!important}.metric-support{font-size:13px!important}}@media(max-width:620px){.metric-projection td{display:block!important;width:auto!important;padding-right:0!important}.metric-projection .metric-label{min-height:0!important}}${responsiveStyles.join('')}</style></head><body style="margin:0;background:#f0f5f3;font-family:Arial,sans-serif;color:#003641"><table role="presentation" width="100%" cellspacing="0" cellpadding="0"><tr><td align="center" style="padding:16px 8px"><table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:680px;background:#fff"><tr><td data-communication-header style="padding:14px 22px;background:#003641;color:#fff"><p style="font-size:12px;margin:0 0 5px;color:#8fdbcf">${escape(header.brand)}</p><h1 style="font-size:23px;margin:0 0 5px;overflow-wrap:anywhere">${escape(header.centralName)}</h1><p style="font-size:16px;margin:0;font-weight:700">${escape(header.periodLabel)}</p></td></tr><tr><td class="panel-content" style="padding:14px 22px;overflow-wrap:anywhere"><div data-communication-context>${[header.unitLabel, header.unitContext].filter(Boolean).length ? p([header.unitLabel, header.unitContext].filter(Boolean).join(' · '), 'font-size:15px;font-weight:700;margin:0 0 5px;') : ''}${model.notes.length ? p(model.notes.join(' · '), 'font-size:12px;color:#435c60;margin:0 0 8px;') : ''}</div>${model.greeting ? p(model.greeting) : ''}${model.opening ? p(model.opening) : ''}${body}${model.footer ? p(model.footer, 'font-size:11px;color:#435c60;') : ''}${model.signature ? p(model.signature) : ''}</td></tr></table></td></tr></table></body></html>`;
}

...
export function renderDashboardHtml(model, subject = '') {
  ...
  const body = model.blocks.map((b) => {
    ...
    if (b.type === 'cards') {
      const cells = b.items.map((i, index) => `<td data-metric="${escape(i.label)}" ${index === b.items.length - 1 && b.items.length % 2 ? 'colspan="2" width="100%"' : 'width="50%"'} valign="top" style="padding:18px;border:1px solid #dbe8e5;background:${i.accent ? '#003641' : '#ffffff'}"><div style="font-size:14px;color:${i.accent ? '#c4dedf' : '#435c60'}">${escape(i.label)}</div><strong style="display:block;margin:8px 0;font-size:26px;line-height:1.2;color:${i.accent ? '#ffffff' : '#003641'};overflow-wrap:anywhere">${escape(i.value)}</strong>${i.support ? `<div style="font-size:16px;line-height:1.5;font-weight:${i.accent ? '600' : '400'};color:${i.accent ? '#ffffff' : '#435c60'}">${escape(i.support)}</div>` : ''}</td>`);
      return `<table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border-collapse:collapse;table-layout:fixed">${cells.map((c, i) => i % 2 === 0 ? `<tr>${c}${cells[i + 1] || ''}</tr>` : '').join('')}</table>`;
    }

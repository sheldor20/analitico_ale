...
const method = 'Dias úteis sem feriados. Dados ausentes não são zero.';
...
export function commercialAction(section, kind) {
  ...
  if (v.gap === 0) return kind === 'central' && section.individualGap > 0
    ? 'Consolidado na meta. Concentre a atuação nas cooperativas que ainda têm GAP a produzir.'
...
function metricCards(values, title, annual, showProjection) {
  ...
  const items = [
    { label: annual ? 'Meta anual' : 'Meta do período', value: money(values.target), support: title },
    { label: 'Realizado informado', value: money(values.actual), support: `${percent(values.attainment)} da meta`, accent: true },
...
export function buildPortfolioPresentation(report, { names = [], intro = '', signature = '', subject = '', showProjection = false } = {}) {
  ...
  for (const section of report.sections) {
    ...
    const individual = entity.kind === 'central' && section.childCount > 1
      ? `GAP para todas as ${section.childCount} cooperativas: ${money(section.individualGap)} (não compensado pela superação de outras unidades).` : '';
    const priorities = section.priorities.map((item) => `${item.name}: ${money(item.gap)}`).join('; ');
    const details = [...(individual ? [individual] : []), ...(priorities ? [`Priorize: ${priorities}.`] : [])];
    if (warning(v)) details.push(warning(v));
    primaryText.push('', section.label, position, headline(v), ...(showProjection ? [projection(v)] : []), ...details, action);
    primaryWhatsapp.push('', `*${section.label}*`, position, headline(v), ...(showProjection ? [projection(v)] : []), ...details, action);
    blocks.push({ type: 'text', text: position, tone: 'muted' });
    blocks.push(metricCards(v, selectedTitle, report.period === 'annual', showProjection));
    for (const text of details) blocks.push({ type: 'text', text, tone: text === warning(v) ? 'warning' : 'body' });
    if (v.requiredDaily > 0) blocks.push({ type: 'text', text: `Necessário por dia útil: ${money(v.requiredDaily)}.`, tone: 'body' });
    blocks.push({ type: 'text', text: action, tone: 'action' });
  }
  // Use the same cards for the annual view; legacy table/secondary snapshots remain readable.
  const annual = [];
  if (report.period !== 'annual') {
    ...
    for (const section of report.sections) {
      ...
      blocks.push({ type: 'text', text: `${cutoff(section)} ${a.phase}.`, tone: 'muted' });
      blocks.push(metricCards(a, `Janeiro a dezembro de ${report.year}`, true, showProjection));
      if (warning(a)) blocks.push({ type: 'text', text: warning(a), tone: 'warning' });
      annual.push(section.label, headline(a), ...(showProjection ? [projection(a)] : []), ...(warning(a) ? [warning(a)] : []));
    }
  }
  const footer = `${showProjection ? 'Projeções são estimativas. ' : ''}${method}`;
  ...
  const end = ['', footer, ...(signature.trim() ? ['', signature.trim()] : [])];
  const text = [greeting, '', opening, hierarchy, `Período: ${selectedTitle}.`, ...notes, ...primaryText, ...annual, ...end].join('\n');
  const whatsapp = [greeting, '', `*${safeName(scope)}*`, hierarchy, `*${selectedTitle}*`, ...(intro.trim() ? [intro.trim()] : []), ...notes, ...primaryWhatsapp, ...annual, ...end].join('\n');
...
export function renderDashboardHtml(model, subject = '') {
  ...
  const body = model.blocks.map((b) => {
    ...
    if (b.type === 'cards') {
      const rows = [];
      for (let index = 0; index < b.items.length; index += 3) {
        const items = b.items.slice(index, index + 3);
        const secondary = index > 0 && items.length === 1;
        const cells = items.map((item) => {
          const valueSize = item.value.length > 20 ? 20 : item.value.length > 15 ? 22 : 26;
          const label = `<div class="metric-label" style="${secondary ? '' : 'min-height:40px;'}font-size:14px;line-height:1.4;color:${item.accent ? '#c4dedf' : '#435c60'}">${escape(item.label)}</div>`;
          const value = `<strong class="metric-value" style="display:block;margin:${secondary ? '4px' : '8px'} 0;font-size:${valueSize}px;line-height:1.2;color:${item.accent ? '#ffffff' : '#003641'};overflow-wrap:anywhere;word-wrap:break-word;word-break:break-word">${escape(item.value)}</strong>`;
          const support = item.support ? `<div class="metric-support" style="font-size:16px;line-height:1.4;font-weight:${item.accent ? '600' : '400'};color:${item.accent ? '#ffffff' : '#435c60'};overflow-wrap:anywhere">${escape(item.support)}</div>` : '';
          const content = secondary ? `<table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="table-layout:fixed"><tr><td width="30%" valign="top" style="padding-right:12px">${label}</td><td width="38%" valign="top" style="padding-right:12px">${value}</td><td width="32%" valign="top">${support}</td></tr></table>` : `${label}${value}${support}`;
          return `<td class="metric-cell" data-metric="${escape(item.label)}" width="${100 / items.length}%" valign="top" style="width:${100 / items.length}%;padding:${secondary ? '12px' : '16px 12px'};border:1px solid #dbe8e5;background:${item.accent ? '#003641' : '#ffffff'};overflow-wrap:anywhere">${content}</td>`;
        });
        rows.push(`<table data-layout="metric-cards" data-columns="${items.length}" role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border-collapse:collapse;table-layout:fixed;${index ? 'margin-top:8px;' : ''}"><tr>${cells.join('')}</tr></table>`);
      }
      return rows.join('');
    }
  ...
  return `<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escape(subject || model.scope)}</title><style>@media(max-width:480px){.panel-content{padding:14px!important}.metric-cell{padding:12px 8px!important}.metric-label{font-size:12px!important;min-height:36px!important}.metric-value{font-size:20px!important}.metric-support{font-size:13px!important}}@media(max-width:359px){[data-layout="metric-cards"]>tbody>tr>.metric-cell{display:block!important;width:auto!important}.metric-label{min-height:0!important}.metric-value{font-size:24px!important}.metric-support{font-size:15px!important}}</style></head><body style="margin:0;background:#f0f5f3;font-family:Arial,sans-serif;color:#003641"><table role="presentation" width="100%" cellspacing="0" cellpadding="0"><tr><td align="center" style="padding:16px 8px"><table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:680px;background:#fff"><tr><td style="padding:24px;background:#003641;color:#fff"><p style="font-size:12px;margin:0 0 8px;color:#8fdbcf">GESTÃO COMERCIAL</p><h1 style="font-size:25px;margin:0 0 10px;overflow-wrap:anywhere">${escape(model.scope)}</h1><p style="font-size:19px;margin:0;font-weight:700">${escape(model.periodLabel)}</p><p style="font-size:12px;margin:10px 0 0">${escape(model.hierarchy)}</p></td></tr><tr><td class="panel-content" style="padding:22px;overflow-wrap:anywhere">${p(model.greeting)}${p(model.opening)}${model.notes.map((n) => p(n, 'font-size:12px;')).join('')}${body}${p(model.footer, 'font-size:11px;color:#435c60;')}${model.signature ? p(model.signature) : ''}</td></tr></table></td></tr></table></body></html>`;
}

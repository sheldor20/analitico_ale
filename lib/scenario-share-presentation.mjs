...
function grouped(rows, kind) {
  const parents = new Map();
  for (const row of rows) { const key = parentKey(row, kind); if (!parents.has(key)) parents.set(key, []); parents.get(key).push(row); }
  return [...parents.values()].flat();
}
...
function reportText(report) {
  ...
  return [`${display.title} · ${display.metricLabel}`, display.scope, `${report.periodLabel}${display.cutoffLabel ? ` · ${display.cutoffLabel}` : ''}`,
    report.selectionLabel, ...(report.intro ? ['', report.intro] : []),
    ...report.notes, '', ...rows, ...(report.cta ? ['', report.cta] : [])].join('\n');
}

function reportCaption(report) {
  const display = scenarioDisplay(report);
  return [report.intro || `${display.title} · ${display.metricLabel}`, display.scope,
    `${report.periodLabel}${display.cutoffLabel ? ` · ${display.cutoffLabel}` : ''}`,
    report.selectionLabel, ...(report.intro ? [display.metricLabel] : []), ...(report.cta ? ['', report.cta] : [])].join('\n');
}
...
export function buildScenarioReport({ dataset, filters = {}, mode = 'all', kind, selectedIds = [], sortBy, customization = {} }) {
  ...
  const order = sortBy ?? filters.sortBy ?? 'gap';
  ...
  const orderedRows = cohort => grouped(sortAnalysis(analysisIndicators(cohort, indicatorOptions), order).map(row => shareRow(row, kind)), kind);
  ...
  if (parentCount > 1) notes.push(`Por ${kind === 'pa' ? 'cooperativa' : 'central'} · ${SORT_OPTIONS[order]}`);
  ...
  const total = Math.ceil(rows.length / PAGE_SIZE), metadata = { kind, metric, title, scope, periodLabel, notes, context, selectionLabel, year: dataset.year };

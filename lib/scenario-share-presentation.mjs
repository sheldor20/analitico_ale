...
import { scopedAnalyses, sortAnalysis, SORT_OPTIONS } from './scenarios.mjs';
...
export function scenarioDisplay(part) {
  const kind = part.kind || 'pa', context = part.context || contextFor(part.rows), groups = new Map();
  for (const row of part.rows) {
    ...
    if (!groups.has(key)) {
      ...
      groups.set(key, { key, label: labels.join(' · '), rows: [] });
    }
  ...
  return { title: part.title || (kind === 'pa' ? 'Cenário dos PAs' : 'Cenário das cooperativas'), metricLabel: part.metric === 'AR' ? 'Arrecadação' : 'Venda Nova', scope: part.scope,
    cutoffLabel: cut, groups: [...groups.values()] };
}
...
function shareRow(row, kind) {
  return { id: kind === 'pa' ? `pa:${row.central}:${row.cooperative}:${row.pa}` : `cooperative:${row.central}:${row.cooperative}`,
...
function reportText(report) {
  ...
  return [`${display.title} · ${display.metricLabel}`, display.scope, `${report.periodLabel}${display.cutoffLabel ? ` · ${display.cutoffLabel}` : ''}`,
    `${report.count} ${report.kind === 'pa' ? report.count === 1 ? 'PA' : 'PAs' : report.count === 1 ? 'cooperativa' : 'cooperativas'}`,
    ...report.notes, '', ...rows].join('\n');
}
...
export function buildScenarioReport({ dataset, filters = {}, mode = 'all', kind }) {
  ...
  if (!dataset || !Array.isArray(dataset.rows) || !Number.isInteger(dataset.year) || dataset.year < 2020 || dataset.year > 2100
    ...
    || !['all', 'filtered'].includes(mode) || !['pa', 'cooperative'].includes(kind)) throw new Error('Selecione um ano, período e escopo válidos para compartilhar o cenário.');
  ...
  const sorted = sortAnalysis(mode === 'all' ? all : filtered, filters.sortBy).map(row => shareRow(row, kind));
  if (new Set(sorted.map(row => row.id)).size !== sorted.length) throw new Error('Há unidades duplicadas na base. Confira a central e os códigos antes de compartilhar.');
  // Group parents once, with the requested order retained inside every group.
  const parents = new Map();
  for (const row of sorted) { const key = parentKey(row, kind); if (!parents.has(key)) parents.set(key, []); parents.get(key).push(row); }
  const rows = [...parents.values()].flat(), context = contextFor(rows), scope = scopeLabel(dataset, filters, context, kind);
  const title = kind === 'pa' ? 'Cenário dos PAs' : 'Cenário das cooperativas', periodLabel = periodTitle(period, month, dataset.year);
  const notes = mode === 'all' ? [] : [`Lista filtrada: ${rows.length} de ${all.length} ${kind === 'pa' ? 'PAs' : 'cooperativas'}.`];
  if (rows.some(row => row.actual == null || row.target == null || row.variance.value == null)) notes.push('— Dado não informado ou sem avaliação.');
  if (parents.size > 1) notes.push(`Por ${kind === 'pa' ? 'cooperativa' : 'central'} · ${SORT_OPTIONS[filters.sortBy] || SORT_OPTIONS.gap}`);
  ...
  const total = Math.ceil(rows.length / PAGE_SIZE), metadata = { kind, metric, title, scope, periodLabel, notes, context, year: dataset.year };
  ...
  const subject = line(`${title} · ${metric === 'AR' ? 'Arrecadação' : 'Venda Nova'} · ${periodLabel} | ${scope}`).slice(0, 300);
  const report = { ...metadata, mode, count: rows.length, allCount: all.length, filteredCount: filtered.length, scopeLabel: scope, subject, rows, parts };
  const text = reportText(report);
  return { ...report, text, whatsapp: text, html: reportHtml(report) };
}

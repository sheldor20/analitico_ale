...
function targetDescription(analyses, source) {
  if (source === "base")
    return [
...
export function buildPartialCommunication({
  ...
  const isPA = source === "cadence";
  if (!analyses.length)
    throw new Error(`Não há ${isPA ? "PAs" : "cooperativas"} na seleção para gerar a comunicação.`);

  const summary = summarize(analyses);
  const unit = isPA ? "PA" : "cooperativa";
  const units = isPA ? "PAs" : "cooperativas";
  ...
  const populationLabel = isPA ? "Cadência Comercial dos PAs" : "Acompanhamento das Cooperativas";
  const body = [
    "Prezados, bom dia!",
    "",
    ...
    `Saldo para que ${isPA ? "todos os PAs" : "todas as cooperativas"} atinjam 100%: ${money(summary.individualGap)}.`,
    ...
    ...(priorities.length
      ...
      : [allMet
          ? `${isPA ? "Todos os PAs" : "Todas as cooperativas"} da seleção estão com a meta atingida.`
    ...
    "",
    ...targetDescription(analyses, source),
    "",
    ...
    `Contamos com a atuação dos responsáveis para acompanhar ${isPA ? "os PAs" : "as cooperativas"} e transformar as oportunidades em produção.`,
  ].join("\n");

  return {
    subject: `Cenário ${scenarioName} · ${isPA ? "Cadência PA" : "Cooperativas"} · ${metricLabel} · ${selectedScope} · ${MONTHS[month] || "Período"}/${year}`,
    body,

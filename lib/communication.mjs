...
export function buildDecisionInsights(analyses = []) {
  ...
  if (priorities.length) {
    ...
    insights.push({
      title: "Concentração do GAP",
      detail: `${largest.map((item) => identity(item)).join("; ")} concentram ${percent(concentrated / individualGap)} do GAP individual (${money(concentrated)})${pending.length ? " entre registros válidos" : ""}. Priorize o acompanhamento desses responsáveis.`,
      tone: "warning",
    });
  }
  ...
  if (monthly != null && monthly > 0)
    insights.push({
      ...
      detail: `${activePriorities.length === 1 ? "A unidade com GAP e prazo aberto precisa produzir" : `As ${activePriorities.length} unidades com GAP e prazo aberto precisam somar`} ${money(monthly)} por mês equivalente, conforme o prazo restante de cada registro. Distribua esse esforço entre os responsáveis.`,
      tone: "neutral",
    });
  return insights;
}
...
export function buildPartialCommunication({
  ...
  const body = [
    ...
    `GAP consolidado para a meta: ${money(summary.gap)}.`,
    `GAP para que ${allUnits} atinjam 100%: ${money(summary.individualGap)}.`,
    ...
    "",
    "Prioridades de atuação (maior GAP individual primeiro):",
    ...(priorities.length
      ? priorities.map((item, index) =>
          `${index + 1}. ${identity(item, source)}: GAP ${money(item.gap)}. ${actionFor(item).text}`,
        )
      : [allMet

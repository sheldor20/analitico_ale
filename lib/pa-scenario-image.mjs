...
function validatePart(part) {
  ...
  if (!part || !Number.isInteger(part.index) || !Number.isInteger(part.total) || part.index < 1 || part.total < part.index ||
    ...
    ['centralName', 'unitLabel', 'phaseLabel'].some(key => part[key] !== undefined && (typeof part[key] !== 'string' || part[key].length > 5000)) ||
...
export function paScenarioImageLayout(part, context) {
  ...
  y += text(display.brand, inset, y, contentWidth, 20, true, '#8fdbcf') + 6;
  y += text(display.centralName, inset, y, contentWidth, 30, true, '#ffffff') + 6;
  y += text(part.periodLabel, inset, y, contentWidth, 23, false, '#ffffff') + 12;
  commands[header].height = y;
  rect(0, y, width, 5, COLORS.brand);
  y += 17;
  if (display.unitLabel) y += text(display.unitLabel, inset, y, contentWidth, 22, true) + 5;
  y += text([display.cutoffLabel, part.phaseLabel, part.selectionLabel, `Parte ${part.index} de ${part.total} · ${unitLabel} ${part.from}–${part.to}`].filter(Boolean).join(' · '), inset, y, contentWidth, 18, false, COLORS.muted) + 9;

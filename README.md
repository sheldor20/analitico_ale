...
Na própria **Agenda**, use **Novo compromisso**, escolha central, cooperativa ou PA e informe os horários. O compromisso aparece no calendário após salvar, sem abrir Cadastro e metas. Não há antecedência mínima: é possível registrar atividades passadas, de hoje ou futuras dentro do ano selecionado, inclusive com duração de um minuto. O fim deve ser posterior ao início; a duração máxima continua em sete dias.
...
Em **Cadência dos PAs → Compartilhar PAs**, prepare uma lista consolidada com **PA, meta, realizado, atingimento e crescimento/GAP**. O período, a central e a cooperativa seguem a seleção da tela. O compartilhamento da lista usa o recorte filtrado, inclusive os registros com dados pendentes; a opção de incluir todos os PAs da hierarquia continua disponível no gerador. O mesmo comando está na lista **PAs desta cooperativa**, na Visão geral. A comunicação contém apenas a fonte Cadência PA, sem somar resultados da cooperativa.
...
Nas visões **Cooperativas** ou **Cadência dos PAs**, ajuste central, cooperativa, PA, período e carteira. **Gerar comunicação** aproveita os filtros e permite escolher cooperativas, PAs ou uma unidade, em e-mail, imagem para WhatsApp ou painel resumido. A ordem inicial é **Maior atingimento**; outra ordenação escolhida nos filtros acompanha a comunicação. A seleção e a ordem podem ser revisadas; assunto, introdução e chamada para ação são editáveis sem alterar os indicadores.

O cabeçalho contém **Gestão comercial · carteira**, **nome da central** e **período**. Abaixo ficam o painel da cooperativa ou PA e os metadados compactos de atualização, situação e seleção. Datas divergentes permanecem junto às unidades correspondentes.

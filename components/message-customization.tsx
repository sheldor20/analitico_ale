...
export function useMessageCustomization({ owner, kind, metric, contextKey, unit, year, period, baseMessage, baseWhatsapp }: { owner: string | null; kind: string; metric: string; contextKey: string; unit: string; year: number; period: string; baseMessage: any; baseWhatsapp: any }) {
  ...
  const message = baseMessage ? { ...baseMessage, text: applyMessageTemplate(values.email, baseMessage.text, context), html: templateHtml(values.email, baseMessage.html, context) } : null;
  const whatsappMessage = baseWhatsapp ? { ...baseWhatsapp, whatsapp: applyMessageTemplate(values.whatsapp, baseWhatsapp.whatsapp, context) } : null;
  const change = (patch: Partial<typeof values>) => { setEdited({ ...values, ...patch }); setFeedback(''); };
  async function save() {
    if (!owner || !supabase || stored?.scope !== scope) return;
  ...
  return { message, whatsappMessage, editor };
}

...
export default function RegistryManager({ dataset, userId, onChange, busy = false, initialAgenda, onDirtyChange, onSavingChange }: RegistryManagerProps) {
  ...
  return <section className="registry-manager" aria-label={`Cadastro e metas de ${dataset.year}`}>
    <details className={profileStyles.agendaDisclosure} aria-label="Agenda das unidades" open={calendarExpanded}>
      ...
      {calendarExpanded && <ConsolidatedAgenda key={`consolidated:${dataset.year}:${userId}`} entities={entities} year={dataset.year} userId={userId} refreshKey={agendaRevision} onOpenEntity={openEntityAgenda} disabled={locked || relationshipDirty || entityForm !== null} onDirtyChange={setConsolidatedDirty} onSavingChange={setConsolidatedSaving} onAppointmentsChange={refreshFromConsolidated} />}
    </details>

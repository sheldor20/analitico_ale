...
export default function LoginForm({ configured }: { configured: boolean }) {
  ...
  return <main className={styles.page}>
    <section className={styles.intro} aria-label="Gestão Comercial Sicoob">
      ...
      <div><p className={styles.eyebrow}>BAHIA & NORDESTE</p><h1>Uma visão clara.<br />Um acesso protegido.</h1><p className={styles.summary}>Metas, resultados e decisões comerciais em um ambiente de acesso restrito.</p></div>
    ...
    <section className={styles.panel} aria-labelledby="login-title">
      <div className={styles.card}><div className={styles.icon}><LockKeyhole size={25} aria-hidden="true" /></div>
        ...
        </form><p className={styles.private}>Importações, cadastros e relatórios estão disponíveis somente após a autenticação. Não há cadastro público.</p>
      </div>
    </section>
  </main>;
}

...
export default function AuthBoundary({ userId, children }: { userId: string; children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    void verify();
    return () => { stopped = true; data.subscription.unsubscribe(); clearInterval(timer); document.removeEventListener('visibilitychange', visible); window.removeEventListener('pageshow', restored); };
  }, [userId]);

...
test('desktop: contact isolation, dashboard, Outlook, WhatsApp, file and saved draft', async ({ page }, testInfo) => {
  ...
  const next = new URL(await dialog.getByRole('link', { name: 'Abrir Outlook', exact: true }).getAttribute('href'));
...
test('central and PA use their own responsible contacts and original level', async ({ page }, testInfo) => {
  ...
  expect(new URL(await composer(page).getByRole('link', { name: 'Abrir Outlook', exact: true }).getAttribute('href')).searchParams.get('to')).toBe('paula@example.com');

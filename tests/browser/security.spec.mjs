...
test('public hashed assets retain cache while private routes stay no-store', async ({page, request}) => {
  await page.goto('/login');
  const src=await page.locator('script[src^="/_next/static/"]').first().getAttribute('src');
  expect(src).toBeTruthy();
  const asset=await request.get(src);expect(asset.status()).toBe(200);
  expect(asset.headers()['cache-control']).toContain('immutable');
  const denied=await request.get('/api/auth/logout');expect(denied.status()).toBe(401);

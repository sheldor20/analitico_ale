...
test('PA zero remains identifiable while shared hierarchy and cutoff appear once, without mutating the report', () => {
  ...
  assert.equal(drawn.filter((command) => command.value.includes('Sicoob Central Bahia')).length, 1);
  ...
  assert.ok(drawn.some((command) => command.value.includes('Parte 1 de 1 · PAs 1–1')));
...
test('twenty PAs fit below 2600 pixels with no missing rows or repeated shared metadata', () => {
  ...
  const measure = measurement(), layout = paScenarioImageLayout(source, measure), drawn = textCommands(layout);
  assert.ok(drawn.some((command) => command.value.includes('Parte 2 de 3 · PAs 21–40')));
...
test('mobile-sized parts show the partial selection count once and preserve every selected financial row', () => {
  ...
  assert.equal(drawn.filter(command => command.value.includes(source.selectionLabel)).length, 1);
  assert.ok(drawn.some(command => command.value.includes('Parte 1 de 2 · PAs 1–12')));
...
test('cooperative collection scenarios accept empty PA identifiers and use cooperative/AR labels', () => {
  ...
  assert.ok(fullText.includes('Gestão comercial · Arrecadação'));
...
test('central scope labels each cooperative group once and preserves identical PA numbers under different parents', () => {
  ...
  assert.ok(second.some((command) => command.value.includes('Sicoob Central Bahia')));
